...
func (f *metadataClient) Get(ctx context.Context) (*clusterctlv1.Metadata, error) {
	...
	// Dump the metadata.yaml content to logs for debugging
	log.V(5).Info("metadata.yaml content", "provider", f.provider.ManifestLabel(), "version", f.version, "content", string(file))
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to parse current version %q for provider %q", f.version, f.provider.ManifestLabel())
	}
